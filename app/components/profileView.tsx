...
const ProfileView = (props: ProfileViewProps) =>{
    return (
        <div className="w-1/3 flex flex-col items-center mt-5 min-w-fit">
            ...
            <div className="bg-gray-800 w-full flex flex-col items-center p-2 rounded-t-2xl">
                <p className="self-start ml-3 text-gray-400">Informações do usuário</p>
                <div className="bg-cyan-600 rounded-full border-[3px] border-gray-400">
                    <img src={`https://raw.githubusercontent.com/Mar-7th/StarRailRes/master/${props.profile?.player?.avatar.icon}` } alt="icon perfil" crossOrigin="anonymous" />
                </div>
            ...
            <div className="bg-gray-700 w-full flex items-center flex-col">
                ...
                <div className="m-5 w-full">
                    ...
                    <div className="mx-9 my-2 py-1 px-6 flex justify-between w-10/12 bg-gray-400 rounded-xl">
                        <p className="pt-0.5">Áreas descobertas</p>
                ...
                <p className="font-bold text-left w-full px-5 mb-3"> ID:{props.profile?.player?.uid}</p>
            </div>
        </div>
    )
}
